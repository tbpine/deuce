...
public class UnitTestsBracketTournament
{
    ...
    public void bracket_to_completion(int noPlayers)
    {
        ...
        for (int currentRound = 1; currentRound <= totalRounds; currentRound++)
        {
            ...
            {
                var winnersRound = winnersDraw.GetRoundIndex(currentRound - 1);
            ...
            {
                var losersRound = losersDraw.GetRoundIndex(currentRound - 1);
            ...
            if (winnersScores.Count > 0 || losersScores.Count > 0)
            {
                ...
                TestContext?.WriteLine($"  Total winners matches: {(currentRound <= winnersDraw.NoRounds ? winnersDraw.GetRoundIndex(currentRound - 1).Permutations.Sum(p => p.Matches.Count()) : 0)}");
                TestContext?.WriteLine($"  Total losers matches: {(currentRound <= losersDraw.NoRounds ? losersDraw.GetRoundIndex(currentRound - 1).Permutations.Sum(p => p.Matches.Count()) : 0)}");
        ...
        var finalWinnersRound = winnersDraw.GetRoundIndex(winnersDraw.NoRounds - 1);
        var finalLosersRound = losersDraw.GetRoundIndex(losersDraw.NoRounds - 1);
    ...
    {
        var winnersDraw = tournament.Draw!;
        var firstRound = winnersDraw.GetRoundIndex(0);
    ...
    private void VerifyBracketStructure(Tournament tournament, int originalPlayers)
    {
        ...
        int totalTeamsInWinners = winnersDraw.GetRoundIndex(0).Permutations
            .SelectMany(p => p.Matches)
